...
export default function CoverLetter({ onBack }) {
  ...
  const generate = async () => {
    ...
    if (
      ...
      alert("Cover Letter limit reached.");
      return;
    }
...
Write a complete cover letter, 3-4 paragraphs, that connects the candidate's real experience to this specific role. Do not invent achievements not present in the resume. Use plain text only, no markdown formatting, ready to copy into an email or document. Include a greeting and sign-off.`;
  ...
  const saveCoverLetter = async () => {
    try {
      ...
      alert("Failed to save cover letter: " + err.message);
    }
  };
  ...
  return (
      <ToolShell onBack={onBack} title="Cover Letter Generator">
        <div style={panelStyle}>
          ...
          <TemplatePicker setTemplate={setTemplate} template={template} />
          ...
          {letter && (
            <div style={{ marginTop: 24 }}>
              ...
              <pre
                style={{
                  background: "#0f172a",
                  border: `1px solid ${(TEMPLATES[template]?.accent || "#3b82f6")}40`,
                  borderRadius: 10,
                  padding: 20,
                  whiteSpace: "pre-wrap",
